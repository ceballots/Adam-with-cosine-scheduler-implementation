...
class AdamLearningRule(GradientDescentLearningRule):
    ...
    def update_params(self, grads_wrt_params):
        ...
        """
        self.step_count += 1
        for param, d_i, mom_1, mom_2 in zip(self.params,grads_wrt_params,self.moms_1,self.moms_2):
            mom_1 *= self.beta_1
            mom_1 += (1-self.beta_1)*d_i
            mom_2 *= self.beta_2
            mom_2 += (1-self.beta_2)*(d_i**2)
            #mom_1_hat = (mom_1/(1-(self.beta_1)**self.step_count))
            #mom_2_hat = (mom_2/(1-(self.beta_2)**self.step_count))
...
class AdamLearningRuleWithWeightDecay(GradientDescentLearningRule):
    ...
    def __init__(self, learning_rate=1e-3, beta_1=0.9, beta_2=0.999,
        ...
        self.learning_rate = learning_rate
        self.eta = self.learning_rate / self.initial_learning_rate
        ##delete
    ...
    def update_params(self, grads_wrt_params):
        ...
        for param, d_i, mom_1, mom_2 in zip(self.params,grads_wrt_params,self.moms_1,self.moms_2):
            ...
            #self.nt = 1.     
            #p = self.nt*(self.weight_decay*param)
            #param -= p
            #param -= self.initial_learning_rate*(mom_1/((mom_2)**(0.5)+self.epsilon))  
            
            p = self.eta*(self.weight_decay*param)
            param -= p
            param -= self.learning_rate*(mom_1_hat/((mom_2_hat)**(0.5)+self.epsilon))  
    ...
    def update_params(self, grads_wrt_params):
        ...
        Args:
            grads_wrt_params: A list of gradients of the scalar loss function
                ...
                previously, with this list expected to be in the same order.
        """
        for param, mom, d_i  in zip(self.params, self.moms, grads_wrt_params):
            mom *= self.beta
            mom += (1-self.beta)*d_i**2
            param -=self.learning_rate*d_i/(mom**(0.5) + self.epsilon)