...
from mlp.schedulers import CosineAnnealingNoWarmRestarts
import numpy as np
import math 
...
def test_cosine_scheduler():
    ...
    cosine_scheduler = CosineAnnealingWithWarmRestarts(min_learning_rate=0.0001, max_learning_rate=0.01,
                                                       ...
                                                       max_learning_rate_discount_factor=.9,
                                                       min_learning_rate_discount_factor =.5,
                                                       period_iteration_expansion_factor=1)
    
    """To test with no warm restarts"""
    #cosine_scheduler = CosineAnnealingNoWarmRestarts(min_learning_rate=0.0001, max_learning_rate=0.05,
    #                                                 total_epochs=100.)
